...
2. **完整输出**：`/mmy` 调用 `fetch.py` 时，**必须完整输出脚本返回的全部内容，包括标题、热度、原文链接，绝不摘要、不省略、不缩写**。每个平台每个条目都要显示，每源显示条目数由 `limit_per_source` 控制（默认10）
3. **避免刷屏**：`snapshot`、`daily`、`open_urls` 只报告结果，不在对话中打印全部内容；只有 `fetch` 才需要打印
4. **会话检测**：如果登录会话过期（返回公开源而非订阅源），提示用户重新登录