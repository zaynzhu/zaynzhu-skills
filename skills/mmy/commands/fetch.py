...
def main() -> int:
    parser = argparse.ArgumentParser(description="显示当前热榜")
    parser.add_argument("--sources", help="覆盖配置的平台（逗号分隔，如 zhihu,weibo）")
    ...
    if args.format == "json":
        print(json.dumps({"fetched_at": fetched_at, "filtered": removed_count, "groups": groups}, ensure_ascii=False, indent=2))
    else:
