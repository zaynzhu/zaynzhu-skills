...
当系统有 Node.js 且 `~/.pet/pet-renderer.mjs` 存在时，自动启用增强模式：
...
渲染器通过 `__dirname` 定位 `pets/` 目录，无需硬编码 skill 安装路径。`~/.pet/` 是唯一的运行时目录，包含所有必要文件。
...
**接食物小游戏：**
- 运行 `node ~/.pet/pet-renderer.mjs --mode=game` 启动
- 左右方向键移动宠物接住食物，躲避障碍物
...
1. `~/.pet/status-combined.sh` 是一个包装脚本
...
原始 statusLine 命令保存在 `~/.pet/original-statusline-cmd.txt`。
安装时会自动备份当前 statusLine 命令到该文件。

**查找优先级：**

1. `~/.pet/original-statusline-cmd.txt` — 优先读取
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "bash ~/.pet/status-combined.sh",
    "refreshInterval": 5
  }
}
```
...
`~/.pet/original-statusline-cmd.txt`，status-combined.sh 会自动执行并追加宠物状态。
...
- 确认 skill 名称 `pet` 与实际 skill 文件名匹配
- 重启 Claude Code 会话
