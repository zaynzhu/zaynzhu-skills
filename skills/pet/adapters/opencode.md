...
1. **项目级**：`.opencode/skills/pet-buddy/SKILL.md`
2. **项目级（Claude 兼容）**：`.claude/skills/pet-buddy/SKILL.md`
3. **全局级**：`~/.config/opencode/skills/pet-buddy/SKILL.md`
4. **全局级（Claude 兼容）**：`~/.claude/skills/pet-buddy/SKILL.md`

OpenCode 会自动发现这些路径中的 skill。触发方式与 Claude Code 相同：

1. **Skill 工具调用**：AI 代理通过 `skill({ name: "pet-buddy" })` 主动加载
...
OpenCode 使用 JS/TS 插件系统而非 bash hook 脚本。Pet Buddy 的插件文件位于 `pet-buddy/hooks/opencode/pet-buddy-plugin.ts`。
...
```json
{
  "plugin": [
    ...
    "~/.config/opencode/plugins/pet-buddy.ts"
  ]
}
```
...
**或项目级部署（随项目版本控制）：**
```bash
cp pet-buddy/hooks/opencode/pet-buddy-plugin.ts .opencode/plugins/pet-buddy.ts
```
...
- 读取和写入 `~/.pet-buddy/` 目录（通过 Node.js fs 模块）
...
- 确认文件名为大写 `SKILL.md`
- 检查 frontmatter 中的 `name` 字段是否为 `pet-buddy`
