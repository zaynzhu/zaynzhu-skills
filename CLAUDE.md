...
- **同作用域 `CLAUDE.md` 与 `AGENTS.md` 默认保持全文一致；修改规则必须同步两份，无法裁定的分歧询问用户，只有用户明确要求时保留差异**
...
- **新增或合并其他机器/会话推入的 skill 后，必须按 A/B 标准补一轮发布物审计（A 真实项目/公司/私人内容，B 机器专属路径），发现即报告、用户裁决后才改**
