...
| 🐾 | [**pet**](./skills/pet/) | CLI 编程伴侣宠物。提供有持久属性的猫/狗宠物，能对工作状态做出实时反应，支持养成升级、状态栏集成和 Hook 自动互动 | `stable` |
| 📡 | [**nanobanana-monitor**](./skills/nanobanana-monitor/) | X/Twitter 热帖监控机器人。搜索最近24小时内指定关键词的高互动帖子，按点赞排序输出 Top 20 速报，关键词可自定义，默认监控 NanoBanana Pro 系列 | `stable` |
...
| `pet` | Bash + jq | 无（Claude Code hooks 可选） | Claude Code（状态栏 + Hook 自动互动） |
| `nanobanana-monitor` | Python ≥ 3.8 | `TAVILY_API_KEY` 环境变量 | tavily-search-enhanced skill |
...
>
> **nanobanana-monitor** 依赖 Tavily API Key，关键词可自定义（不仅限于 NanoBanana Pro，也支持 gemini3、OpenAI image 2.0 等任意关键词）
