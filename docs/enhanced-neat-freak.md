...
- 强制前置：第零步尺寸体检、第一步记忆机械式全量枚举；每个文件完整读取并逐条检查 description、绝对时间、自然失效和记忆间矛盾，完成前重新枚举对账。
- 证据账本：同时接收本次增量和存量审查发现，每条判断都有证据。最近更新过不能作为 keep 的依据。
- 范围收窄只减少项目级 docs 扫描深度，记忆审查始终保留；不可读取或未核实项必须报告为未完成。
- 跨工具规则：同作用域 CLAUDE.md / AGENTS.md 默认全文一致，识别实际别名；无法裁定的分歧询问用户，不假装已同步不可访问的机器。
